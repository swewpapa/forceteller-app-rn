...
## Phase B — 활성화 (2026-07-13, Martin 확정)

메커니즘(Phase A) 위에 `ROUTE_GUARDS.Web`를 켬. **범위 = 계정 전용 페이지만.**

- `GUARDED_WEB_PATH_PREFIXES` = `/force` `/freeforce` `/giftbox` `/fatebook` `/profile/update` `/push` (leading-slash, 프리픽스 경계 매칭). 근거: 레거시 Angular `AuthGuard` 적용 모듈 중 **계정 필수** 페이지. auth 없으면 기능 자체가 성립 안 해 게이트가 정상 동작.
- **의도적 제외(콘텐츠성)**: `/celeb` `/dream` `/charm` `/lucky-items` `/luck-level` `/donation` `/cal` — 레거시는 게이트했으나 게스트 브라우징을 막지 않으려 보류. 필요 시 `GUARDED_WEB_PATH_PREFIXES`에 추가.
- `ROUTE_GUARDS.Web.requiresAuth` = `(params) => webPathRequiresAuth(params.path)` predicate. Web params는 구조적 타입으로 판단(features 간 import 금지).
- 브랜치: `feature/auth-guard-phase-b`(PR #23 위 스택).

**남은 것(Martin 실기기 QA)**: 게스트가 계정 링크 탭 → Login 모달 → 구글 로그인 → 원래 경로 복귀 / 모달 취소 → Tabs 이탈. iOS 구글 로그인 네이티브 동작 확인 포함(Android는 세션 중 e2e 확인됨).
