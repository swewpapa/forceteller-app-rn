...
- **계층별 네이밍 언어**: API 메서드/훅은 API-친화적(`listByCode`, `useThemeListByCode`) — "어떻게 조회하는가". 타입/컴포넌트는 도메인 개념(`Theme`, `ThemeView`) — "무엇인가". HTTP 라우트 어휘를 프레젠테이션에 새지 않게.
- **theme 도메인 어휘**: 엔티티 = `Theme`(두 컨텍스트 공유), 컴포넌트 = `ThemeWidget*`(홈 위젯 UI). 컨텍스트 구분은 훅 이름(`listByCode` vs 향후 `getById`)이 한다. `ThemeList` 타입은 중의적이라 만들지 않는다(응답은 `Theme[]`).
